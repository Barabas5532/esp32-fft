...
void fft_test_task() {
  ...
  for (n = MIN_LOG_N; n <= MAX_LOG_N; n++) {
    ...
    for (k = 0; k < fft_analysis->size; k++) {
      ...
      fft_analysis->input[2 * k + 1] =
          (float)(k - 1) / (float)fft_analysis->size;
    }
    ...
    for (k = 0; k < 2 * fft_analysis->size; k++)
      if (fabsf(fft_analysis->input[k] - fft_synthesis->output[k]) > 1e-5f) {
        printf("bin=%d input=%.4f output=%.4f\n err=%f", k,
               fft_analysis->input[k], fft_synthesis->output[k],
